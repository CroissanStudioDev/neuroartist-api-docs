...
const PAGES = [
  ...
  ['Каталог моделей: обзор', 'generate/index.mdx'],
  ['Каталог: поиск', 'generate/search.mdx'],
  ['Каталог: схема модели', 'generate/schema.mdx'],
  ['API: обзор', 'api/index.mdx'],
  ['API: /run', 'api/run.mdx'],
  ['API: /queue', 'api/queue.mdx'],
  ['API: /me', 'api/me.mdx'],
  ['API: /billing', 'api/billing.mdx'],
  ['API: /webhooks/yookassa', 'api/webhooks.mdx'],
...
export async function GET() {
  ...
  const body = [
    ...
    'API-ключ, рублёвый биллинг, async-очередь, S3-хранение результатов.',
    '',
    ...
    '- Основные задачи: image generation → `/generate/images` и `/build/sync`; long-running через async-очередь → `/build/async`; top-up → `/manage/billing` и `/manage/auto-topup`.',
    '',
    '---',
    '',
    ...sections
