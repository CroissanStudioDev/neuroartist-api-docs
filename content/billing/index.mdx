...
- **Списание** атомарно при `POST /run` или `POST /queue/{modelId}` — `creditCost` модели (integer, ≥ 1 ₽). Не хватает — `402 insufficient_balance`, **генерация не запускается**.
- **Возврат** — автоматически при ошибке генерации.
