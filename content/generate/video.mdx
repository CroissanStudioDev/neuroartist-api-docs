...
Видео-модели — от 30 секунд до нескольких минут. Используйте **async + polling** или **webhooks**, sync-режим (`/run`) для них почти всегда отвалится по 5-минутному таймауту.
...
- ❌ Для batch-генерации используйте webhook, а не polling в цикле — надёжнее при большом числе задач.
