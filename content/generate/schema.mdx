...
| Поле | Описание |
| --- | --- |
| `creditCost` | Цена за «типовую» 1 единицу в ₽ (admin-настройка, integer). Используется как fallback fixed-price для моделей без `priceUnit`. |
...
Списание точное: `priceRub × quantity` с учётом скидки, округление вверх до **0.0001 ₽** (минимальной единицы учёта). Никакого пола в 1 ₽ нет — субкопеечные модели остаются субкопеечными.

Для моделей с динамической ценой (per-second video, per-megapixel image) реальное списание уточняется после генерации — reconciliation возвращает разницу как `refund` (микроразницы ≤ 0.01 ₽ игнорируются). См. [Биллинг](/billing#жизненный-цикл-денег).

Точный расчёт перед запуском с реальным input — `POST /models/{modelId}/estimate`. В ответе будут оба значения: `estimatedCostRub` (сколько спишется, decimal ₽) и `estimatedPriceRub` (цена за единицу до округления).
