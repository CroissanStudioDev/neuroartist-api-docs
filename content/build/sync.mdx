...
| Признак | Решение |
| --- | --- |
| Часто > 60 секунд | [Асинхронная очередь и polling](/build/async) |
| Reverse-proxy режет соединения | [Async submit + worker](/build/async#serverless--edge-рантаймы) |
| Хотите параллельные запросы | [`/queue`](/api/queue) |

## Дальше

- [Асинхронная очередь и polling](/build/async) — для генераций, не помещающихся в HTTP-таймауты.
